...
// genId() produces prefixed IDs such as:
// skill_xxxxx, proj_xxxxx
// Milestones may also use IDs such as m3.
  ...
  return db
    .prepare(
      `SELECT
         ...
         m.label,
         CASE
           WHEN um.user_id IS NOT NULL THEN 1
           ELSE 0
         END AS completed
       FROM milestones m
...
function getRecognitionsFor(userId) {
  return db
    ...
    .map((row) =>
      s.recognition(
        row,
        row.from_name ? { name: row.from_name } : null
      )
    );
}
